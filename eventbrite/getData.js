...
const URLS_FILE_PATH = './data/eventbrite-urls.txt';
...
async function main() {
  try {
    const urlsData = await readFileAsync(URLS_FILE_PATH, 'utf-8');
    const urls = urlsData.split('\n').filter(url => url.startsWith('https://www.eventbrite.com'));
    for (let url of urls) {
      scraper.addTarget({
        url,
        func: grabRequest
